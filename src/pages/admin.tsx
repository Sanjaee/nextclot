...
// Frontend base URL - selalu gunakan zascript.com
...
export default function AdminPage() {
  ...
  // Helper functions untuk generate frontend URLs yang benar
  const getFrontendEditUrl = (uuid: string) =>
    `${FRONTEND_BASE_URL}/edit/${uuid}`;
  const getFrontendViewUrl = (uuid: string) =>
    `${FRONTEND_BASE_URL}/scan/${uuid}`;
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        // Generate frontend URL yang benar untuk toast
  ...
  const toggleUserStatus = async (userId: string) => {
    try {
      // Gunakan relative URL
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/users/${userId}/toggle-status`,
        {
          method: "PUT",
        }
      );
  ...
  const regenerateQR = async (uuid: string) => {
    try {
      // Gunakan relative URL
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/qr/${uuid}/regenerate`,
        {
          method: "POST",
        }
      );
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge
                            variant={
                              user.qrProfile.isPublished ? "default" : "outline"
                            }
                            className="text-xs"
                          >
                        ...
                        {/* URLs - Gunakan frontend URLs yang benar */}
                        <div className="space-y-1">
                          <div className="flex gap-2">
                            <Button
                              ...
                              onClick={() =>
                                copyToClipboard(
                                  getFrontendEditUrl(user.qrProfile!.uuid)
                                )
                              }
                            >
                        ...
                        {/* Display URLs untuk referensi */}
                        <div className="text-xs text-gray-500 space-y-1">
                          <div>
                            Edit: {getFrontendEditUrl(user.qrProfile.uuid)}
                          </div>
                          {user.qrProfile.isPublished && (
                            <div>
                              View: {getFrontendViewUrl(user.qrProfile.uuid)}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-2">
                        {user.qrProfile.qrCode && (
                          <img
                            ...
                            onClick={() =>
                              downloadQR(user.qrProfile!.qrCode!, user.username)
                            }
